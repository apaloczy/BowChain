...
% Shorten timeseries to the appropriate ranges skipping nans
tinds1 = find(time1 >= trange(1) & time1 <= trange(2) & ~isnan(signal1));
tinds2 = find(time2 >= trange(1) & time2 <= trange(2) & ~isnan(signal2));
...
plot(time1(tinds1(1:leni)),signal1(tinds1(1:leni)),...
  ...
  time2(tinds2(1:leni))+offset,signal2(tinds2(1:leni)))
